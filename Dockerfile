...
RUN cd $HOME/setup && R CMD INSTALL swan

### run test scripts ### 
RUN cd $HOME/setup/swan/test && if [ ! -e "swan_test.tgz" ]; then wget https://s3-us-west-2.amazonaws.com/lixiabucket/swan_test.tgz -O swan_test.tgz 2>&1 >/dev/null; fi && tar -zxvf swan_test.tgz
